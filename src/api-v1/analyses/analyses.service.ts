...
import { MAX_OFFSHORE_KM, coastFor } from '../dataspace/coastline';
...
function normalizeAnalyses(
  reqAnalyses: AnalysesRunRequest['analyses'],
): AnalysisId[] {
...
function locationProfile(
  ...
  mpSeasonAmp: number;
  seasonPhase: number;
  mpTrendPerDay: number;
  bcfBase: number;
  bcfRange: number;
} {
  const h = hashString(
    `profile|${Math.round(lat * 10)}|${Math.round(lon * 10)}`,
  );
...
export class AnalysesService {
  ...
  async run(
    ...
    // Which stretch of coast the request is on decides which datasets may answer
    // it. A point on no coast has none, and serving it the least-distant dataset
    // anyway would return figures that read as measurements of somewhere they
    // are not — so it is refused rather than answered.
    const match = coastFor(req.location);
    if (!match) {
      throw new BadRequestException(
        `location ${req.location.lat},${req.location.lon} is not within ` +
          `${MAX_OFFSHORE_KM} km of a covered coastline ` +
          `(Mediterranean, Atlantic — Gulf of Cádiz and the Canaries — or Cantabrian)`,
      );
    }

    const aggregationMode: AggregationMode =
      req.aggregation?.mode === 'monthly' ? 'monthly' : 'raw';
    ...
    const dateRangeApplied =
      req.dateRange?.start && req.dateRange?.end
        ? {
            start: dateToIsoDay(req.dateRange.start),
            end: dateToIsoDay(req.dateRange.end),
          }
        : { start: '2025-01-01', end: '2025-01-30' };

    const cacheMode: CacheMode = req.options?.cache?.mode ?? 'reuse';
    const ttlSeconds: number =
      req.options?.cache?.ttlSeconds ?? 30 * 24 * 60 * 60; // 30d
    ...
    if (
      (cacheMode === 'recompute' || cacheMode === 'reuse') &&
      !skipCacheForWebp
    ) {
  ...
  private async computeResponse(
    ...
    ctx: {
      ...
      /** The coast whose observed datasets may answer this request. */
      coast: Ocean;
    ...
    const totalDays = daysInclusive(
      dateRangeApplied.start,
      dateRangeApplied.end,
    );
    const dates =
    ...
    const pseudoLocations = [
      { locationId: 'loc_target', lat: location.lat, lon: location.lon },
    ];
    const weightsByLocId = new Map([['loc_target', 1]]);

    const polymers = ['PE', 'PP', 'PET', 'PS'] as const;

    // Nearest datasets on this coast: observed where they exist, the calibration
    // series otherwise. Non-blocking: null on any error.
    const s3 = await this.safeLoadS3Data(req.location, ctx.coast);
    ...
    const s3BiomassStd = Math.min(
      s3?.biomass?.stdTonnes ?? 2.5,
      s3BiomassBase * 0.5,
    );
    ...
    for (const loc of pseudoLocations) {
      for (const d of dates) {
        ...
        const rr = mulberry32(
          hashString(`${loc.locationId}|${loc.lat}|${loc.lon}|${d}`),
        );
        ...
        if (s3MpBase !== null) {
          mpByKey.set(
            key,
            Math.max(0.01, s3MpBase + (mpNoise - 0.5) * 2 * s3MpStd),
          );
        } else {
          ...
          mpByKey.set(
            key,
            Math.max(0.01, mpBase / 10 + mpAmp * 0.3 + mpNoise * 2),
          );
        }

        biomassByKey.set(
          key,
          Math.max(0.1, s3BiomassBase + (rr() - 0.5) * 2 * s3BiomassStd),
        );
        kgTotalByKey.set(
          key,
          Math.max(0.1, s3KgBase + (rr() - 0.5) * 2 * s3KgStd),
        );
        envFactorByKey.set(
          key,
          clamp(s3EnvBase + (rr() - 0.5) * 2 * s3EnvStd, 0.5, 1.5),
        );
      }
    }
    ...
    if (ctx.selectedAnalyses.includes('basic_contamination')) {
      ...
      for (const d of dates) {
        ...
        for (const loc of pseudoLocations) {
          ...
          pairs.push({
            value: mp,
            weight: weightsByLocId.get(loc.locationId) ?? 0,
          });
        }
      ...
      const concordSeed = hashString(
        `concord|${targetLoc.lat}|${targetLoc.lon}|${dateRangeApplied.start}|${dateRangeApplied.end}`,
      );
      const rc = mulberry32(concordSeed);
      const buoyPolymers: string[] =
        s3?.buoyPolymers ??
        (polymers.filter(() => rc() > 0.25) as unknown as string[]);
      const waterPolymers: string[] =
        s3?.water?.polymers ??
        (polymers.filter(() => rc() > 0.1) as unknown as string[]);
      const overlapPercent = Number(
        jaccardPercent(buoyPolymers, waterPolymers).toFixed(1),
      );
          ...
          byLocationAndDateRange: [
            {
              location: targetLoc,
              dateRange: {
                start: dateRangeApplied.start,
                end: dateRangeApplied.end,
              },
              buoyPolymers,
              waterPolymers,
              overlapPercent,
    ...
    if (ctx.selectedAnalyses.includes('trophic_transfer')) {
      ...
      for (const d of dates) {
        ...
        for (const loc of pseudoLocations) {
          ...
          const daySeed = hashString(
            `fish|${loc.locationId}|${loc.lat}|${loc.lon}|${d}`,
          );
        ...
        bcfItems.push({
          location: targetLoc,
          date: d,
          BCF: Number(weightedMean(pairs).toFixed(2)),
        });
      }

      // Polymer similarity: build aggregated water/fish % per date, then pearson per polymer.
      const waterPercByPolyAgg: Record<(typeof polymers)[number], number[]> = {
        PE: [],
        PP: [],
        PET: [],
        PS: [],
      };
      const fishPercByPolyAgg: Record<(typeof polymers)[number], number[]> = {
        PE: [],
        PP: [],
        PET: [],
        PS: [],
      ...
      for (const d of dates) {
        const waterPairsByPoly: Record<
          (typeof polymers)[number],
        ...
        for (const loc of pseudoLocations) {
          const daySeed = hashString(
            `poly|${loc.locationId}|${loc.lat}|${loc.lon}|${d}`,
          );
          const rr = mulberry32(daySeed);
          const wts = randomWeights(rr, polymers.length);

          const fishNoiseScale = 0.4;
          const fRaw = wts.map((x) =>
            Math.max(0.001, x * (1 + (rr() - 0.5) * fishNoiseScale)),
          );
      ...
      const polymersResult: Record<
        string,
        { pearson_r: number; p_value: number }
      > = {};
      for (const p of polymers) {
        const { r, p: pval } = pearsonCorrelation(
          waterPercByPolyAgg[p],
          fishPercByPolyAgg[p],
        );
        polymersResult[p] = {
          pearson_r: Number(r.toFixed(2)),
          p_value: Number(pval.toFixed(3)),
        };
      }
    ...
    if (ctx.selectedAnalyses.includes('eco_risk')) {
      const exposureItems: Array<{
        location: { lat: number; lon: number };
        date: string;
        Exposure_Index: number;
      }> = [];
      const pressureItems: Array<{
        location: { lat: number; lon: number };
        date: string;
        Plastic_Pressure_Index: number;
      }> = [];
      const ipcItems: Array<{
        location: { lat: number; lon: number };
        date: string;
        IPC: number;
      }> = [];
      ...
      for (const d of dates) {
        ...
        for (const loc of pseudoLocations) {
          const mp = mpByKey.get(`${loc.locationId}|${d}`) ?? 0.1;
          const daySeed = hashString(
            `env|${loc.locationId}|${loc.lat}|${loc.lon}|${d}`,
          );
          const rr = mulberry32(daySeed);
        ...
        exposureItems.push({
          location: targetLoc,
          date: d,
          Exposure_Index: Number(weightedMean(exposurePairs).toFixed(3)),
        });
        pressureItems.push({
          location: targetLoc,
          date: d,
          Plastic_Pressure_Index: Number(
            weightedMean(pressurePairs).toFixed(3),
          ),
        });
        ipcItems.push({
          location: targetLoc,
          date: d,
          IPC: Number(weightedMean(ipcPairs).toFixed(3)),
        });
      }
    ...
    if (ctx.selectedAnalyses.includes('plastic_origin')) {
      ...
      for (const d of dates) {
        ...
        for (const loc of pseudoLocations) {
          const mp = mpByKey.get(`${loc.locationId}|${d}`) ?? 0.1;
          const daySeed = hashString(
            `coast|${loc.locationId}|${loc.lat}|${loc.lon}|${d}`,
          );
          const rr = mulberry32(daySeed);
          const key = `${loc.locationId}|${d}`;
          const kg_total = kgTotalByKey.get(key) ?? rr() * 150;
        ...
        csiItems.push({
          location: targetLoc,
          date: d,
          CSI: Number(weightedMean(pairs).toFixed(6)),
        });
      }
    ...
    const wantFormatted =
      req.options?.dataFormattedForPlots === true ||
      req.options?.savePlotsWebp === true;
    if (wantFormatted) {
    ...
    if (req.options?.savePlotsWebp === true && response.dataFormattedForPlots) {
      response.plotWebpPaths = await savePlotsAsWebp(
  ...
  private buildPlotsFormatted(ctx: {
    ...
    for (const d of dates) {
      ...
      for (const loc of pseudoLocations) {
        ...
        const fishNoiseScale = 0.4;
        const fRaw = wts.map((x) =>
          Math.max(0.001, x * (1 + (rrPoly() - 0.5) * fishNoiseScale)),
        );
    ...
    for (let i = 0; i < polymers.length; i++) {
      ...
      for (let j = 0; j < polymers.length; j++) {
        const r = pearsonCorrelation(
          waterPercSeries[polymers[i]],
          waterPercSeries[polymers[j]],
        ).r;
    ...
    const concordSeed = hashString(
      `concord|${coords.lat}|${coords.lon}|${dates[0]}|${dates[dates.length - 1]}`,
    );
    const rc = mulberry32(concordSeed);
    const buoyPolymers: string[] =
      ctx.buoyPolymers ?? polymerLabels.filter(() => rc() > 0.25);
    const waterPolymers: string[] =
      ctx.waterPolymers ?? polymerLabels.filter(() => rc() > 0.1);
    const overlapPercent = Number(
      jaccardPercent(buoyPolymers, waterPolymers).toFixed(1),
    );

    const waterVec = polymerLabels.map((p) =>
      mean(waterPercSeries[p as (typeof polymers)[number]]),
    );
    const fishVec = polymerLabels.map((p) =>
      mean(fishPercSeries[p as (typeof polymers)[number]]),
    );
    const { r: simR, p: simP } = pearsonCorrelation(waterVec, fishVec);
    return {
      locationId: locLabel,
  ...
  private async safeLoadS3Data(
    loc: { lat: number; lon: number },
    coast: Ocean,
  ): Promise<S3Scenario | null> {
    try {
      return await this.scenario.load(loc, coast);
    } catch {
      return null;
    }
  }
