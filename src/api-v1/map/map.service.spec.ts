...
function service(assetsList: unknown[], cleanupRows: unknown[] = []) {
  const assets = {
    find: jest.fn().mockResolvedValue(assetsList),
  } as unknown as AssetsRepository;
  const observations = {
    cleanupRows: jest.fn().mockResolvedValue(cleanupRows),
...
describe('MapService.getPoints', () => {
  ...
  it('computes bounds across markers', async () => {
    const { svc } = service([
      ...
      ASSET({
        _id: 'a2',
        sourceId: '71034599-0830-44a8-a4bd-5f7aea885536',
        location: { type: 'Point', coordinates: [-5.7212, 43.5721] },
      }),
    ]);
    const res = await svc.getPoints();
  ...
  it('hides assets that never completed an ingest', async () => {
    const { svc } = service([
      ASSET({ currentIngestId: null }),
      ASSET({ _id: 'a3', status: 'failed' }),
    ]);
    const res = await svc.getPoints();
  ...
  it('still serves an asset the space no longer offers, with a warning', async () => {
    const { svc } = service([ASSET({ status: 'missing' })]);
    const res = await svc.getPoints();
    expect(res.count).toBe(1);
    expect(
      res.points[0].warnings.some((w) => /offers this asset any more/.test(w)),
    ).toBe(true);
  });
